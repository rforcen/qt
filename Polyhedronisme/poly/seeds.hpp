...
public:
    ...
    {
        return Polyhedron("C", {{0.707, 0.707, 0.707}, {-0.707, 0.707, 0.707}, {-0.707, -0.707, 0.707}, {0.707, -0.707, 0.707}, {0.707, -0.707, -0.707}, {0.707, 0.707, -0.707}, {-0.707, 0.707, -0.707}, {-0.707, -0.707, -0.707}},
                          {{3, 0, 1, 2}, {3, 4, 5, 0}, {0, 5, 6, 1}, {1, 6, 7, 2}, {2, 7, 4, 3}, {5, 4, 7, 6}});
    }
    static Polyhedron icosahedron()
    {
        return Polyhedron("I", {{0, 0, 1.176}, {1.051, 0, 0.526}, {0.324, 1.0, 0.525}, {-0.851, 0.618, 0.526}, {-0.851, -0.618, 0.526}, {0.325, -1.0, 0.526}, {0.851, 0.618, -0.526}, {0.851, -0.618, -0.526}, {-0.325, 1.0, -0.526}, {-1.051, 0, -0.526}, {-0.325, -1.0, -0.526}, {0, 0, -1.176}}, {{0, 1, 2}, {0, 2, 3}, {0, 3, 4}, {0, 4, 5}, {0, 5, 1}, {1, 5, 7}, {1, 7, 6}, {1, 6, 2}, {2, 6, 8}, {2, 8, 3}, {3, 8, 9}, {3, 9, 4}, {4, 9, 10}, {4, 10, 5}, {5, 10, 7}, {6, 7, 11}, {6, 11, 8}, {7, 10, 11}, {8, 11, 9}, {9, 11, 10}});
    }
    static Polyhedron octahedron()
    {
        return Polyhedron("O", {{0, 0, 1.414}, {1.414, 0, 0}, {0, 1.414, 0}, {-1.414, 0, 0}, {0, -1.414, 0}, {0, 0, -1.414}}, {{0, 1, 2}, {0, 2, 3}, {0, 3, 4}, {0, 4, 1}, {1, 4, 5}, {1, 5, 2}, {2, 5, 3}, {3, 5, 4}});
    }
    static Polyhedron dodecahedron()
    {
        return Polyhedron("D", {{0, 0, 1.07047}, {0.713644, 0, 0.797878}, {-0.356822, 0.618, 0.797878}, {-0.356822, -0.618, 0.797878}, {0.797878, 0.618034, 0.356822}, {0.797878, -0.618, 0.356822}, {-0.934172, 0.381966, 0.356822}, {0.136294, 1.0, 0.356822}, {0.136294, -1.0, 0.356822}, {-0.934172, -0.381966, 0.356822}, {0.934172, 0.381966, -0.356822}, {0.934172, -0.381966, -0.356822}, {-0.797878, 0.618, -0.356822}, {-0.136294, 1.0, -0.356822}, {-0.136294, -1.0, -0.356822}, {-0.797878, -0.618034, -0.356822}, {0.356822, 0.618, -0.797878}, {0.356822, -0.618, -0.797878}, {-0.713644, 0, -0.797878}, {0, 0, -1.07047}},
    ...
    static Polyhedron prism(int n = 4)
    {
        ...
        Vertexes vertexes;
        for (int i = 0; i < n; i++)
            vertexes.push_back(Vertex{-cosf(i * theta), -sinf(i * theta), -h});
        for (int i = 0; i < n; i++)
            vertexes.push_back(Vertex{-cosf(i * theta), -sinf(i * theta), h});
    ...
    static Polyhedron antiprism(int n = 4)
    {
        ...
        for (int i = 0; i < n; i++)
            vertexes.push_back(Vertex{r * cosf(i * theta), r * sinf(i * theta), h});
        for (int i = 0; i < n; i++)
            vertexes.push_back(Vertex{r * cosf((i + 0.5) * theta), r * sinf((i + 0.5) * theta), -h});
    ...
    {
        auto p = johnsons[j];
        return Polyhedron("J" + to_string(j), p.second, p.first);
    }
    ...
    static Polyhedron polyhedron(char name = 'T', int n = 0, float alpha = 0, float height = 0)
    {
        ...
        case 'A':
            return antiprism(n);
        case 'U':
            return cupola(n, alpha, height);
        case 'V':
            return anticupola(n, alpha, height);
        case 'J':
            return johnson(n);
        default:
            return Polyhedron();
...
public: // utils
    ...
    {
        const int n = 10;
        for (auto p : {tetrahedron, cube, icosahedron, octahedron, dodecahedron})
            p().print();
        for (auto p : {pyramid, prism, antiprism})
            p(n).print();
        for (auto p : {cupola, anticupola})
            p(n, 0, 0).print();
        for (int i = 0; i < 92; i++)
            johnson(i + 1).print();
    }
