    ...
    Thread(poly.n_faces).run([&poly, &flags, dist, &normals](int t, int i)
                             {
                               ...
                               for (auto &v2 : f)
                               {
                                 ...
                                 flag.add_vertex(i4(v2), poly.vertexes[v2] * (1.0f + dist));
                                 // Add a new vertex, moved parallel to normal.
                                 auto v2new = i4(i, v2);

                                 flag.add_vertex(v2new, poly.vertexes[v2] + (normals[i] * dist * 1.5f));
  ...
  static Polyhedron trisub(Polyhedron &poly, int n = 2)
  {
    ...
    for (size_t fn = 0; fn < poly.faces.size(); fn++)
    {
      ...
      for (int i = 0; i <= n; i++)
      {
        for (int j = 0; j + i <= n; j++)
        {
          auto v = (v1 + v21 * (i * 1.0 / n)) + (v31 * n / j * 1.0);
