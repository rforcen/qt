...
public:
  ...
  Common(QObject *parent = nullptr) : QObject(parent) {
    srand(uint(QTime::currentTime().msec())); // random seed
  ...
  QStringList sample_rate_values() {
    QStringList sl;
    for (auto &v : _sample_rate_values)
      sl << QString::number(v);
    return sl;
  }
  ...
  void generate_mt() { // scale & fill audio buffer buff w/evaluation
    buff->zero();
    ...
    if (compiler.is_ok()) {
      ...
      vector<VSL_compiler> compilers(Thread::getnthreads());
      for (auto &c : compilers)
        c.compile(expression.toStdWString());
      vector<Scaler<double>> scalers(Thread::getnthreads());
      int n_values = int(n_channels * secs * sample_rate);
      vector<float> snd(n_values);
  ...
  QString paste_from_clip() { // spread sheet oriented paste
    ...
    if (!s.isEmpty()) {
      ...
      for (int il = 1; il < lines.size(); il++) { // skip header
        auto slahp = lines[il].split('\t', Qt::SkipEmptyParts);
        if (slahp.size() == 3) {
