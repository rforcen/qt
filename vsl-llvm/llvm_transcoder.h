...
double freq_mesh(double base, double slope, double islope, double n, double t,
                 FreqMesh *fm) {
  if (!fm->is_init())
    fm->set_param(base, slope, uint(islope), uint(n));
...
public:
  ...
  intptr_t get_function(string name) {
    auto addr = engine->getFunctionAddress(name);
    return addr;
  }
...
public:
  ...
  int generate(Value *t_value, Value *nth_value) {
    ...
    case POWER:
      if (stack.size() >= 2) {
        stack.bin_oper(
            Builder.CreateCall(create_func(sym), {stack.prev(), stack.top()}));
      } else
        err = true;
      break;
  ...
  Function *create_func(int sym) { // create internal func on demand by 'sym'
    ...
    if (funcs.find(sym) == funcs.end())
      ...
      case NOTE_CONST:
        ...
        break;
      case SEQUENCE:
        funcs[sym] = gen_func("sequence",
                              {dbl_type(), dbl_type(), dbl_type(), ptr_type()});
        break;
