...
void Common::generate_llvm_mt() {
  ...
  if (compiler.is_ok()) {
    ...
    vector<Scaler<double>> scalers(n_threads);
...
void Common::generate_llvm_st() {
  ...
  if (compiler.is_ok()) {
    ...
    if (transcoder->ok()) {
      ...
      for (auto i = 0, isamp = 0; i < iget_n_samples(); i++) {
        double t = t_inc * i; // thread timing

        for (int ch = 0; ch < n_channels; ch++) {
          double y = transcoder->evaluate(t, ch, 0);
          scalers[0].update(y);
          snd[isamp++] = float(y);
