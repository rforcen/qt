...
class VSL_compiler : public AuxFunc {
  ...
  public:
    ...
    Table_Values(wstring id, types type, int address, int n_params)
        : id(id), sid(ws2s(id)), type(type), address(address),
          n_params(n_params) {} // FUNC
    Table_Values(wstring id, types type, int param_ix, int i_func,
                 int _dummy) // PARAM
        : id(id), sid(ws2s(id)), type(type), param_ix(param_ix),
          i_func(i_func) {
      (void)_dummy;
    }
  ...
  public:
    ...
    vector<string> get_lets_ids() {
      ...
      for (auto &tb : tab_values)
        if (tb.type == Table_Values::NUM_ID)
          vs.push_back(tb.sid);
      return vs;
    }
    ...
    Table_Values get_by_address(int addr) {
      for (auto &tb : tab_values)
        if (tb.address == addr && tb.type == Table_Values::FUNC)
          return tb;
      return Table_Values();
    }
    ...
    // try
    {
      for (int pc = from_pc; pc < to_pc && !err_number;) {
        ...
        case FREQ_MESH: { // *(base, slope, islope, n)
          pc++;
          FreqMesh &fm =
              compiler->freq_mesh[size_t(*reinterpret_cast<int *>(code + pc))];
          pc += sizeof(int);
        ...
        case NOTE_CONST:
          pc++;
          _stack.push(sin(
              NoteOct2Freq(*reinterpret_cast<int *>(code + pc),
