...
void Queen::saveSolution() { // in queens
  if (!m_abort) {
    ...
    if (stopSolutions != 0 && solutions.size() >= stopSolutions)
      abort();
  }
}
...
QString Queen::getSolution(int ix) {
  QString s;
  if (ix >= 0 && ix < solutions.size())
    s = toString(solutions[ix]);
  return s;
}
...
QString Queen::toStringRaw() {
  QString s;
  for (int i = 0; i < nQueens; i++)
    s += QString::number(queens[i]);
  return s;
}
QString Queen::toStringRaw(int *q) {
  QString s;
  for (int i = 0; i < nQueens; i++)
    s += QString::number(q[i]);
  return s;
}
...
int *Queen::fromString(QString s) { //  to queens
  auto sl = s.split(" ", Qt::SkipEmptyParts);
  if (sl.size() == nQueens) {
    for (int i = 0; i < nQueens; i++)
      queens[i] = sl[i].toInt();
  }
