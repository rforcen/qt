...
public:
  ...
  void setSolution(const QModelIndex &index) {
    auto qs = data(index).toString().split(" ", Qt::SkipEmptyParts);
    if (qs.size() == q->nQueens)
      for (int i = 0; i < q->nQueens; i++)
        q->queens[i] = qs[i].toInt() - 1;
  }
  void setSolution() {
    auto qs = data(index(0, 0)).toString().split(" ", Qt::SkipEmptyParts);
    if (qs.size() == q->nQueens)
      for (int i = 0; i < q->nQueens; i++)
        q->queens[i] = qs[i].toInt() - 1;
  }
  ...
  QVariant headerData(int section, Qt::Orientation orientation,
    ...
    case Qt::Vertical:
      if (role == Qt::DisplayRole)
        return QVariant(section * nCols + 1);
      break;
    default:;
    }
    return QVariant();
