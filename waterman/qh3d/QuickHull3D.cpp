...
QuickHull3D::~QuickHull3D() { cleanup(); }
 ...
 */
double QuickHull3D::getDistanceTolerance() { return tolerance; }
 ...
 */
double QuickHull3D::getExplicitDistanceTolerance() { return explicitTolerance; }
 ...
 */
QuickHull3D::QuickHull3D() { initPrt(); }
...
void QuickHull3D::build(vector<double> coords, int nump) {
  ...
  assert((coords.size() / 3 >= (size_t)nump) &&
...
void QuickHull3D::triangulate() {
  ...
  for (Face *face = newFaces->first(); face != nullptr; face = face->next) {
    faces.push_back(face);
  }
}
...
void QuickHull3D::computeMaxAndMin() {
  ...
  if (explicitTolerance == AUTOMATIC_TOLERANCE) {
    tolerance = 3 * DOUBLE_PREC *
                (std::max<double>(fabs(max->x), fabs(min->x)) +
                 std::max<double>(fabs(max->y), fabs(min->y)) +
                 std::max<double>(fabs(max->z), fabs(min->z)));
  } else {
...
void QuickHull3D::createInitialSimplex() {
  ...
  assert(!(sqrt(maxSqr) <= 100 * tolerance) &&
  ...
  assert(!(fabs(maxDist) <= 100 * tolerance) &&
 ...
 */
int QuickHull3D::getNumVertices() { return numVertices; }
 ...
 */
int QuickHull3D::getNumFaces() { return (int)faces.size(); }
 ...
 */
vector<vector<int>> QuickHull3D::getFaces() { return getFaces(CCW); }
...
void QuickHull3D::resolveUnclaimedPoints(FaceList *newFaces) {
  ...
  for (Vertex *vtx = vtxNext; vtx != nullptr; vtx = vtxNext) {
    vtxNext = vtx->next;

    double maxDist = tolerance;
    Face *maxFace = nullptr;
    for (Face *newFace = newFaces->first(); newFace != nullptr;
...
bool QuickHull3D::doAdjacentMerge(Face *face, int mergeType) {
  ...
  do {
    ...
    if (mergeType ==
        NONCONVEX) { // then merge faces if they are definitively non-convex
...
void QuickHull3D::calculateHorizon(Point3d *eyePnt, HalfEdge *edge0, Face *face,
...
void QuickHull3D::addNewFaces(FaceList *newFaces, Vertex *eyeVtx,
                              HalfEdgeVector horizon) {
  newFaces->clear();

  HalfEdge *hedgeSidePrev = nullptr;
  HalfEdge *hedgeSideBegin = nullptr;

  for (auto horizonHe : horizon) {
    HalfEdge *hedgeSide = addAdjoiningFace(eyeVtx, horizonHe);

    if (hedgeSidePrev != nullptr) {
...
Vertex *QuickHull3D::nextPointToAdd() {
  if (!claimed->isEmpty()) {
    ...
    Vertex *eyeVtx = nullptr;
    double maxDist = 0;
    for (Vertex *vtx = eyeFace->outside; vtx != nullptr && vtx->face == eyeFace;
         vtx = vtx->next) {
...
void QuickHull3D::addPointToHull(Vertex *eyeVtx) {
  ...
  calculateHorizon(eyeVtx->pnt, nullptr, eyeVtx->face, horizon);
  newFaces->clear();
 ...
 * set to <code>nullptr</code> if no messages are desired.
 * @return true if the hull is valid
 * @see QuickHull3D#check(PrintStream,double)
 */
bool QuickHull3D::check() { return check(getDistanceTolerance()); }
