...
public:
  ...
  allocate(size_type size,
    ...
    void *voidPointer =
        clSVMAlloc(context_(), SVMTrait::getSVMMemFlags(), size * sizeof(T), 0);
...
#if CL_HPP_TARGET_OPENCL_VERSION >= 200 ||                                     \
    defined(CL_HPP_USE_CL_IMAGE2D_FROM_BUFFER_KHR)
...
public:
  ...
  void setSVMPointersHelper(std::array<void *, ArrayLength> &pointerList,
                            const pointer<T0, D> &t0, const pointer<T1, D> &t1,
                            Ts &... ts) {
  ...
  cl_int setExecInfo(cl_kernel_exec_info param_name, const T &val) {
    return detail::errHandler(
        ::clSetKernelExecInfo(object_, param_name, sizeof(T), &val));
  }

  template <cl_kernel_exec_info name>
  cl_int setExecInfo(typename detail::param_traits<detail::cl_kernel_exec_info,
                                                   name>::param_type &val) {
...
public:
  ...
  /**
   * Program constructor to allow construction of program from SPIR-V or another
   * IL. Valid for either OpenCL >= 2.1 or when CL_HPP_USE_IL_KHR is defined.
   */
  ...
  /**
   * Program constructor to allow construction of program from SPIR-V or another
   * IL for a specific context. Valid for either OpenCL >= 2.1 or when
   * CL_HPP_USE_IL_KHR is defined.
   */
  ...
  cl_int build(const Device &device, const char *options = NULL,
    ...
    cl_int buildError =
        ::clBuildProgram(object_, 1, &deviceID, options, notifyFptr, data);
  ...
  CL_API_PREFIX__VERSION_2_2_DEPRECATED cl_int setReleaseCallback(
    ...
    return detail::errHandler(
        ::clSetProgramReleaseCallback(object_, pfn_notify, user_data),
  ...
  setSpecializationConstant(cl_uint index, const T &value) {
    return detail::errHandler(::clSetProgramSpecializationConstant(
  ...
  cl_int setSpecializationConstant(cl_uint index, size_type size,
                                   const void *value) {
    return detail::errHandler(
        ::clSetProgramSpecializationConstant(object_, index, size, value),
        __SET_PROGRAM_SPECIALIZATION_CONSTANT_ERR);
  }
#endif // CL_HPP_TARGET_OPENCL_VERSION >= 220
};
...
public:
    ...
    if (error != CL_SUCCESS) {
      ...
      if (useWithProperties) {
        cl_queue_properties queue_properties[] = {
            CL_QUEUE_PROPERTIES, static_cast<cl_queue_properties>(properties),
            0};
  ...
  cl_int enqueueWriteImage(const Image &image, cl_bool blocking,
                           ...
                           const void *ptr, const vector<Event> *events = NULL,
                           Event *event = NULL) const {
    cl_event tmp;
    cl_int err = detail::errHandler(
  ...
  cl_int enqueueMigrateSVM(const cl::vector<cl::pointer<T, D>> &svmPointers,
    ...
    return enqueueMigrateSVM(svmPointers,
                             cl::vector<size_type>(svmPointers.size()), flags,
                             events, event);
  }
  ...
  enqueueMigrateSVM(const cl::vector<cl::vector<T, Alloc>> &svmContainers,
    ...
    return enqueueMigrateSVM(svmContainers,
                             cl::vector<size_type>(svmContainers.size()), flags,
                             events, event);
  }
...
public:
  ...
  DeviceCommandQueue(
      ...
      DeviceQueueProperties properties = DeviceQueueProperties::CLNone,
      cl_int *err = NULL) {
    cl_int error;
   ...
   * This can update the default command queue for a device repeatedly to
   * account for kernels that rely on the default.
